/**
 * Firebase Rules Deployment Script (Fixed)
 * Uses service account JWT for Firebase Rules API
 */
...
async function deployFirestoreRules(token) {
  ...
  // Try PUT first (for update)
  let releaseResult = await makeRequest({
    hostname: 'firebaserules.googleapis.com',
    port: 443,
...
async function deployFirestoreIndexes(token) {
  ...
  for (const index of config.indexes || []) {
    ...
    const result = await makeRequest({
      ...
      port: 443,
      path: `/v1/projects/${PROJECT_ID}/databases/(default)/collectionGroups/${collectionGroup}/indexes`,
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json',
  ...
  console.log(`✓ Deployed ${deployed} indexes\n`);
  return true;
}
...
async function deployStorageRules(token) {
  ...
  const rulesetName = rulesetResult.data.name;
  console.log(`✓ Created storage ruleset: ${rulesetName}`);
...
async function verifyDeployment(token) {
  ...
  try {
    // Check Storage rules
    const storageResult = await makeRequest({
      hostname: 'firebaserules.googleapis.com',
      port: 443,
      path: `/v1/projects/${PROJECT_ID}/releases/cloud.storage.release`,
      method: 'GET',
